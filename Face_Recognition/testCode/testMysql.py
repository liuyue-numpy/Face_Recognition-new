...
def predict(image):
    ...
    db = pymysql.connect(host='203.176.95.7', port=3306, user='root', passwd='123123ms@', database='FaceBank',
                         charset='utf8')
    # 使用cursor()方法获取操作游标
    cursor = db.cursor()
...
def get_frame():
    ...
    for row in dat:
        result = {}
        result['Advance'] = dat[7]
        result['Arrival'] = dat[6]
        result['Boarding_port'] = dat[5]
        result['FLT_No'] = dat[4]
        result['FLT_Date'] = dat[3]
        result['age'] = dat[2]
        result['ID_Number'] = dat[1]
        result['name'] = dat[0]
