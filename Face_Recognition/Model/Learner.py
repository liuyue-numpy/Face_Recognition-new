...
import sys
sys.path.append("..")
from testCode.verifacation import evaluate
import torch
...
class face_learner(object):
    def __init__(self, conf, inference=False):
        ...
        # conf.use_mobilfacenet = False，故运行这里
        else:
            # 提取网络层的特征
        ...
        if not inference:
            ...
            if conf.use_mobilfacenet:
                ...
                # 优化算法，weight_decay是正则化，在一定程度上减少模型过拟合的问题
                self.optimizer = optim.SGD([
...
#         self.writer.add_scalar('{}_far:False Acceptance Ratio'.format(db_name), far, self.step)
    ...
    def infer(self, conf, faces, target_embs, tta=False):
        ...
        tta : testCode time augmentation (hfilp, that's all)
        '''
        embs = []
        for img in faces:
            if tta:
